...
import { normalizePath } from 'vite-plus' // Using Vite's normalizePath
...
function generateTypeScriptCode(
	...
	const fileList = sortedFiles.length > 0 ? sortedFiles.map((file) => `  '${file}'`).join(' |\n') : 'never'
...
export default function staticAssetsPlugin(options: StaticAssetsPluginOptions = {}): Plugin {
	...
	const ensureOutputFile = async () => {
		...
		try {
			...
			throw new Error(
				`[vite-plugin-static-assets] Failed to ensure output file/directory (${normalizePath(path.relative(process.cwd(), outputFile))}): ${err instanceof Error ? err.message : err}`,
			)
		}
	}
	...
	return {
		...
		async buildStart() {
			try {
				...
				const files = fs.existsSync(directory) ? await getAllFiles(directory, directory, ignorePatterns) : []

				currentFiles = new Set(files)
				const code = generateTypeScriptCode(files, basePath, {
					...options,
					enableDirectoryTypes,
				}) // Pass resolved options
						...
						const updateTypeScriptFile = async (eventType: updateTypeScriptFileEvent) => {
							...
							debounceTimer = setTimeout(async () => {
								try {
									...
									const updatedCode = generateTypeScriptCode(updatedFiles, basePath, {
										...options,
										enableDirectoryTypes,
									})
