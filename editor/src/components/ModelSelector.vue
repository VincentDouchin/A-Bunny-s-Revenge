...
<template>
	<NTabs :value="selectedCategory ?? undefined" @update:value="val => selectedCategory = val">
		<NTabPane v-for=" models, category in assetStore.models " :key="category" :name="category" :tab="category">
			<NScrollbar style="width: 100%" x-scrollable>
				<NList style="display: flex;" hoverable :show-divider="false">
					<NPopover
						v-for="[name, model] in entries(models).toSorted((a, b) => a[0].localeCompare(b[0]))" :key="name"
					...
					>
						<template #trigger>
							...
							>
								<NEllipsis strong style="width: 100px;">
									{{ name }}
								</NEllipsis>
								<NFlex>
								...
								<NButton
									class="edit-button"
									circle size="small" style="position: absolute;bottom: 1rem;right: 1rem"
									@click="() => {
										mode = 'model'
										selectedModel = name
									}"
								...
								<Thumbnail :thumbnail-key="`${category}-${name}`" :model />
							</NListItem>
						</template>
						<NTag v-for="value, tag in (modelDataStore.modelData?.[category]?.[name]?.tags ?? {})" :key="tag" type="error" :bordered="false">
							{{ tag }} - {{ value }}
						</NTag>
					</NPopover>
				</NList>
