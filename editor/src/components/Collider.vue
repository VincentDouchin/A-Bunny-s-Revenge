...
const scale = computed<[number, number, number]>(() => {
	switch (collider.value.type) {
		case 'cuboid': return [collider.value.size.x, collider.value.size.y, collider.value.size.z]
		case 'capsule':
		case 'cylinder': return [collider.value.size.x, collider.value.size.y, collider.value.size.x]
		case 'ball': return [collider.value.size.x, collider.value.size.x, collider.value.size.x]
		default: return [1, 1, 1]
	}
})

const body = computed((prev?: RigidBody) => {
	prev && props.world.removeRigidBody(prev)
...
const colliderDesc = computed(() => {
	...
	switch (collider.value.type) {
		case 'ball': return ColliderDesc.ball(collider.value.size.x / 2)
		case 'capsule': return ColliderDesc.capsule(collider.value.size.x / 2, collider.value.size.y)
		case 'cuboid': return ColliderDesc.cuboid(collider.value.size.x / 2, collider.value.size.y / 2, collider.value.size.z / 2)
		case 'cylinder': return ColliderDesc.cylinder(collider.value.size.y / 2, collider.value.size.x / 2)
	}
	return null
})
...
<template>
	...
	>
		<TresMesh
			...
			:position="[0, collider.type === 'capsule' ? 1 : 0.5, 0]"
		>
			<TresBoxGeometry v-if="collider.type === 'cuboid'" :args="[1, 1, 1]" />
			<TresSphereGeometry v-else-if="collider.type === 'ball'" :args="[0.5]" />
			<TresCylinderGeometry v-else-if="collider.type === 'cylinder'" :args="[0.5, 0.5]" />
			<TresCapsuleGeometry v-else-if="collider.type === 'capsule'" :args="[0.5]" />
			<TresMeshBasicMaterial :color transparent :opacity="0.2" />
		</TresMesh>
	</TresGroup>
</template>
