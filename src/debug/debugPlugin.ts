...
const spawnLevelData = async () => {
	for (const [entityId, data] of Object.entries(await get('levelData') as LevelData)) {
...
export const debugPlugin = (state: State) => {
	state
		.addSubscriber(addDebugCollider)
		.onEnter(spawnLevelData)
}