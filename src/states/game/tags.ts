...
const spawnLamp = () =>
	ecs.with('lamp', 'model').onEntityAdded.subscribe((e) => {
...
const spawnDoorFarm = () =>
	ecs.with('door', 'collider').onEntityAdded.subscribe((e) => {
		e.collider.setSensor(true)
	})

const spawnDoorsDungeon: SubscriberSystem<typeof app, 'dungeon'> = (resources) =>
	ecs.with('doorDungeon', 'position', 'rotation').onEntityAdded.subscribe((e) => {
		ecs.remove(e)
		ecs.add({
...
const gardenPlots = () =>
	ecs.with('gardenPlot', 'collider', 'entityId', 'model').onEntityAdded.subscribe((e) => {
		e.collider.setSensor(true)
...
const well = () =>
	ecs.with('well').onEntityAdded.subscribe((e) => {
		ecs.update(e, {
...
const doorClearing = () =>
	ecs
		.with('doorType', 'doorLevel')
		.where((e) => e.doorType === 'vine')
		.onEntityAdded.subscribe((e) => {
			ecs.addComponent(e, 'doorLocked', true)
		})

export const spawnTagsPlugin: Plugin<typeof app> = (app) => {
	app.addSubscribers('game', spawnLamp, spawnDoorFarm, gardenPlots, well).addSubscribers('dungeon', spawnDoorsDungeon).addSubscribers('clearing', doorClearing)
}
