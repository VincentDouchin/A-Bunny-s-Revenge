...
let intensity = 0
const tween = new Tween({ intensity: 0 }).to({ intensity: 1 }, 60_000 * 10).yoyo(true).repeat(Number.POSITIVE_INFINITY).start(time.elapsed).onUpdate(value => intensity = value.intensity).easing(Easing.Quadratic.InOut)
