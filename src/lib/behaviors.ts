...
export const sequence: NodeProcessor =
	(...nodes) =>
	(e) => {
...
export const selector: NodeProcessor =
	(...nodes) =>
	(e) => {
		for (const node of nodes) {
			const status = node(e)
...
export const parallel: NodeProcessor =
	(...nodes) =>
	(e) => {
		for (const node of nodes) {
			const status = node(e)
...
export const runNodes: NodeProcessor =
	(...nodes) =>
	(e) => {
		for (const node of nodes) {
			node(e)
...
export const condition =
	<E>(check: (e: E) => any): BehaviorNode<E> =>
	(entity) =>
		check(entity) ? 'success' : 'failure'
export const action =
	<E>(execute: (e: E) => any): BehaviorNode<E> =>
	(entity) =>
		execute(entity) === false ? 'failure' : 'success'
export const ifElse =
	<E>(conditionNode: BehaviorNode<E>, successNode: BehaviorNode<E>, failureNode: BehaviorNode<E>): BehaviorNode<E> =>
	(...e) => {
		return conditionNode(...e) === 'success' ? successNode(...e) : failureNode(...e)
	}

export const createBehaviorTree =
	<E extends Entity, C>(query: Query<E>, context: (entity: E) => C, tree: BehaviorNode<C>) =>
	() => {
...
export const waitFor =
	<R>(fn: (args: R) => boolean): BehaviorNode<R> =>
	(args: R) =>
		fn(args) ? 'success' : 'running'
