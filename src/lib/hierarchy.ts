...
export const addChildren = () =>
	parentQuery.onEntityAdded.subscribe((entity) => {
...
export const removeChildren = () =>
	parentQuery.onEntityRemoved.subscribe((entity) => {
		entity.parent.children?.delete(entity)
	})

export const deSpawnChildren = () =>
...
const onDestroyCallBack = () =>
	onDestroyQuery.onEntityRemoved.subscribe((e) => {
		e.onDestroy.emit()
	})
...
export const addToScene =
	(...components: Array<Exclude<ComponentsOfType<Object3D>, 'group'>>): Plugin<typeof app> =>
	(app) => {
		for (const component of components) {
			const query = ecs.with(component, 'position')
			const withoutGroup = query.without('group')
			app.addSubscribers('default', () =>
				sceneQuery.onEntityAdded.subscribe((e) => {
					e.scene.add(e.group)
				}),
			)
			app.addSubscribers('default', () =>
			...
			app.addSubscribers('default', () =>
				withGroup.onEntityRemoved.subscribe((entity) => {
