...
export const TouchButton = <T extends string>({
	input,
	controller,
	children,
	interactable,
	...
	children?: JSXElement
	interactable?: Accessor<{ text: string; icon?: Component } | undefined>
	text?: string
	size: string
	distance?: string
	angle?: string
	...
	css /* css */ `
		.input {
			...
			background: var(--gold);
			box-shadow:
				inset 0.5rem 0.5rem 0 0 var(--gold-shiny),
				0 var(--pressed) 0 0 black;
			opacity: var(--ui-opacity);
		}
		:global(.input svg) {
			filter: drop-shadow(-0.1rem -0.1rem 0rem black) drop-shadow(0.2rem -0.1rem 0rem black) drop-shadow(0.2rem 0.2rem 0rem black) drop-shadow(-0.1rem 0.2rem 0rem black);
		}
		.input-text {
			position: absolute;
			color: white;
	...
	const icon = createMemo(() => {
		if (children && interactable) {
			return interactable()?.icon
		}
	})
			...
			{(player) => {
				...
				const moveCenter = (e: TouchEvent) => {
					...
					if (cont) {
						...
						const center = new Vector2(containerBoundingBox.left + containerBoundingBox.width / 2, containerBoundingBox.top + containerBoundingBox.height / 2)
						...
						if (touch) {
							...
							const input = newPos
								.clone()
								.normalize()
								.multiplyScalar(newPos.length() / max)
				...
				css /* css */ `
					...
					.pause-button {
						...
						border-radius: 1rem;
						box-shadow:
							inset 0.2em 0.2em 0 0 var(--gold-shiny),
				...
				return (
					<div>
						...
						<TouchButton
							input="primary"
							controller={gameInputs.touchController!}
							interactable={primaryInteractable}
							size="10rem"
						/>
						<TouchButton
							input="secondary"
